...
export class Entity extends EntityData {
    constructor() {
        ...
        this.previousState = "";
        this.runDirection = new THREE.Vector3();
    }
    _playAnimation(action, speed = 1, loop = true, callback) {
        let name = action;
        if (this.reference.animation?.[name])
            name = this.reference?.animation[name];
        if (Array.isArray(name))
            name = Random.pick(...name);
        let anim = this.reference.resource.load.animations.find(anim => anim.name == name);
        if (this.object3d.anims.mixer.timeScale != speed)
            this.object3d.anims.mixer.timeScale = speed;
    ...
    displace(position, removePrevious = false) {
        if (removePrevious)
            this.targetLocationList = [];
    ...
    addPhysics() {
        ...
        this.object3d.body.on.collision((otherObject, event) => {
            if (event == 'collision')
                this.emit('collision', { object: otherObject });
    ...
    setHealth(health, ping = true) {
        this.health = health;
        if (ping)
            this.sendHealthUpdateToServer();
        this.emit('health');
    }
    jump() {
    ...
    update() {
        if (this.attackTarget) {
            this.moveTowardsTarget(this.attackTarget.position ? xyzTv(this.attackTarget.position) : this.attackTarget.object3d.position);
        }
    ...
    setInventory(inventory, send = true) {
        this.inventory = inventory;
        this.emit('inventory', { type: 'full' });
        if (send)
            this.sendInventoryUpdateToServer(null, null, 'full', 'full');
    }
    // Method to remove an item from the inventory
    removeFromInventory(item, count = 1, send = true) {
    ...
    sendInventoryUpdateToServer(item, count, type, action) {
    ...
    receiveInventoryUpdateFromServer(update, updatedInventory) {
    }
}
