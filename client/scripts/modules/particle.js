...
export class Particle {
    ...
    scatter() {
        this.mesh.position.add(new THREE.Vector3(Math.random() * 2 - 1, Math.random() * 2 - 1, Math.random() * 2 - 1).normalize().multiplyScalar(this.scatterAmount));
    }
}
export class ParticleSystem {
    constructor(scene) {
        this.particleRegistry = {};
    ...
    particle(name, pos, customData) {
        ...
        if (particle.count > 1)
            for (let i = 0; i < particle.count; i++)
                this.addParticle(particle.scatter());
        else
        ...
        this.register(json.name, (pos, customData) => {
            ...
            if (json.object) {
                ...
                if (json.object.rotation)
                    setVector3Var(o, 'rotation', json.object.rotation, customData);
                if (json.object.position)
                    setVector3Var(o, 'position', json.object.position, customData);
                if (json.object.scale)
                    setVector3Var(o, 'scale', json.object.scale, customData);
                obj.add(o);
            }
            else if (json.text) {
                ...
                if (json.text.position)
                    txt.position.set(json.object.position.x, json.object.position.y, json.object.position.z);
                obj.add(txt);
            }
            const material = materialParser(json.material, this.scene, customData);
            obj.traverse((s) => (s.isMesh) ? s.material = material : null);
            if (pos)
