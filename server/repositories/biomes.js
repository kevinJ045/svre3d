...
export class Biomes {
    ...
    static getBiome(x, z, flags) {
        ...
        if (biome.reference.biome.tile?.multicolor) {
            ...
            if (biome.reference.biome.tile.keepdefault)
                colors.unshift('none');
        ...
        if (biome.reference.biome?.ground?.substitutes && biome.reference.biome.ground.mapping) {
            ...
            const substitute = Random.pick(...substitutes.filter(i => i.chance ? (seedrng() > i.chance) : true), () => Math.abs(noise.simplex2(x * 0.01, z * 0.01)));
            biome.reference.biome.ground.mapping = biome.reference.biome?.ground.mapping
                ...
                .split(',');
            if (Array.isArray(substitute.flags))
                biome.flags.push(...substitute.flags);
            if (substitute.variation_color)
                biome.reference.biome.tile.variation_color = substitute.variation_color;
        }
