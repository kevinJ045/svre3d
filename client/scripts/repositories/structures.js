...
export function generateWithRule(item, seed, object_rules, looted) {
...
function createObject(rule, item, side, seed, parent, looted) {
    let position = new THREE.Vector3(...rule.position);
    if (!rule.size.length)
        rule.size = Array(3).fill(0);
    ...
    if (o) {
        if (side)
            position = getPositionForKey(side, parent, rule, seed);
        else if (Array.isArray(rule.position[0]))
            position = getRandomPos(rule.position, seed);
        if (!rule.size.includes(0))
            o.scale.set(...size);
...
function getRandomPos(position, seed) {
    return new THREE.Vector3(Random.from(position[0][0], position[0][1], seed), Random.from(position[1][0], position[1][1], seed), Random.from(position[2][0], position[2][1], seed));
}
...
export class Structures {
    static createStructureTemplate(structureProps, resource, chunk, variables) {
        const g = new THREE.Group();
        ...
        waterMaterial.uniforms.resolution.value.set(window.innerWidth * 1, window.innerHeight * 1);
        ...
        if (chunk.structures.length) {
            chunk.structures.forEach(structureData => {
                ...
                if (structureProps.view?.animation) {
                    Items.initItemAnimation({ reference: structureProps }, structure);
                }
                ...
                structure.children.forEach(item => {
                    if (item.userData.rule) {
                        if (item.userData.rule.physics === true) {
                            PhysicsManager.addPhysics(item, {
                                shape: 'convex',
                                mass: 0
                            });
                        }
                        if (rule.loot)
                            item.userData.lootable = true;
