...
export class Items {
    static filterItems() {
        Items.items = ResourceMap.all()
            .filter(i => i.manifest?.type == 'item');
    }
    static find(id) {
        return Items.items.find(i => i.manifest.id == id);
    }
    static create(id, quantity = 1, data) {
    ...
    static fromRecipe(preview = false, tool, ...items) {
        ...
        let item = this.craftable().find(i => {
            ...
            if (itemsIds.length < itemsr.length)
                itemsIds.push('empty');
            else if (itemsIds.length > itemsr.length)
                itemsIds.indexOf('empty') ? itemsIds.splice(itemsIds.indexOf('empty')) : null;
            return itemsr.join(',') === itemsIds.sort().join(',');
        });
        if (tool == 'brush')
            item = items[0].reference;
        if (!item)
            return null;
        ...
        if (tool !== 'brush')
            for (const { item: recipeItem, quantity: recipeItemCount } of recipe) {
                ...
                if (!item || item.quantity < recipeItemCount)
                    return null;
            }
        return {
            quantity,
            ...
            if (recipe) {
                if (tool !== 'brush')
                    for (const { item: recipeItem, quantity: recipeItemCount } of recipe.recipe) {
                        ...
                        if (!item?.reference.item?.isTool)
                            entity.removeFromInventory(item, recipeItemCount);
                    }
                if (tool !== 'brush')
                    entity.addToInventory(Items.create(recipe.item.manifest.id, recipe.quantity));
                if (tool == 'brush') {
