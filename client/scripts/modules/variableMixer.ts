...
export function parseVariable(string: string, variables: Record<string, any> = {}){
	variables = {...variables, ...basicVariables};
	return typeof string == "string" ? string.replace(/\$([A-Za-z0-9_]+)/g, (_, name) => variables[name] || _).replace(/([A-Za-z0-9]+)\(([^)]+)\)/g, (_, name, args) => variables[name] ? variables[name](...args.split(',')) : _) : string;
}
