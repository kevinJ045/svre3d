...
export class Equipments {
    static entity(entity) {
        if (!entity.data.equipment.brow)
            entity.data.equipment.brow = 'i:normal-brow';
        for (let i in entity.data.equipment) {
            let item = i == 'brow' ? Items.create({
                itemID: entity.data.equipment[i]
            }) : entity.findItemByData('wid', entity.data.equipment[i]);
            const ref = item?.reference;
            if (ref?.equipment?.type == i) {
                Equipments.equip(entity, i, item);
            }
            if (i == 'brow') {
        ...
        if (ref.config?.material) {
            ...
            if (typeof mat == 'string') {
                ...
                for (let i in mat) {
                    ...
                    const part = Equipments.entityBody(i, { ...item, object3d: equipmentMesh });
    ...
    static updateFlags(type, item, entity) {
        const ref = item?.reference;
        if (ref.item?.flags) {
            const flags = ref.item?.flags;
            const excludeFlags = [];
            for (let i in (entity.data.equipment || {})) {
    ...
    static unequip(entity, type, item) {
    ...
    static entityBody(partname, entity, fallback = '') {
