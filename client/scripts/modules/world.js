...
import { applyMaterials, makeSegmentMaterial } from './shaderMaterial';
...
export function getChunkType(noiseValue, chunkSize, loadedChunks) {
...
function loadChunk(chunkPosition, { chunkSize, loadedChunks }) {
    const scale = 0.0001;
    const offset = 0;
            ...
            item.children.forEach(item => {
                if (item.userData.rule) {
                    if (item.userData.rule.physics === true) {
                        loadedChunks.scene.physics.add.existing(item, {
                            shape: 'convex',
                            mass: 0
                        });
                    }
...
function spawnEntity(loadedChunks, rule, variables, pos) {
    const entity = loadedChunks.scene.entities.summon(rule.entity, '', pos);
    if (rule.variant) {
        const variant = (entity.entityData.config?.variants || []).find(v => v.name == rule.variant);
        if (variant)
            entity.setVariant(variant, variables);
    }
}
