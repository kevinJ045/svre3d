...
export class Lights {
    ...
    static updateLightPosition(v) {
        const { directionalLight } = this.lights;
        if (!directionalLight.userData.offset)
            directionalLight.userData.offset = directionalLight.position.clone();
        const pos = v.clone().sub(directionalLight.target.position);
        directionalLight.target.position.add(pos);
        directionalLight.target.updateMatrixWorld();
        const pos2 = v.clone().sub(directionalLight.position);
        directionalLight.position.add(pos2.add(directionalLight.userData.offset));
