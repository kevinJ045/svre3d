...
export class Entities {
    static spawn(type, position, name, variant, inventory, data, exp) {
        const ref = type == 'item' ? {
            manifes: {
                id: 'm:item',
                type: 'entity'
        ...
        if (!ref)
            return;
        const entity = ServerData.create(EntityData, {
            position,
            name,
            variant,
            type,
        ...
        if (ref.base?.damage)
            entity.damage = ref.base?.damage;
        if (ref.base?.defense)
            entity.defense = ref.base?.defense;
        if (ref.entity?.flags) {
    ...
    static spawnItem(item, position) {
        this.spawn('item', position, item.id, undefined, undefined, { item });
    }
    static kill(entity) {
        if (typeof entity == 'string')
            entity = Entities.find(entity);
        ...
        if (entity.inventory.length) {
            entity.inventory.forEach(item => {
                this.spawnItem(item, entity.position);
            });
        }
        this.entities.forEach(e => {
            if (e.attackTarget?.id == entity.id) {
                e.attackTarget = null;
    ...
    static hp(id, hp) {
        this.entities.forEach(entity => {
            if (entity.id == id) {
                if (entity.health.current > hp.current)
                    entity.emit('hurt');
                else if (entity.health.current < hp.current)
                    entity.emit('heal');
                entity.health = hp;
    ...
    static startPing(socket, serverdata) {
        ...
        pingFrom(socket, 'entity:inventory', ({ entity: eid, type, item, action, full, inventory }) => {
            const entity = Entities.find(eid);
            if (entity) {
                if (full) {
                    ...
                    entity.addToInventory(Items.create(item.type, item.quantity)
                        .setData({ id: item.id, data: item.data }));
                }
                else if (type == 'remove') {
                    entity.removeFromInventory(Items.create(item.type)
                        .setData({ id: item.id, data: item.data }), item.quantity);
                }
    ...
    static selectRandomTarget(entity) {
        ...
        if (ai) {
            if (ai.movement?.biome) {
                chunks = chunks.filter(chunk => chunk.biome.manifest.id == (ai.movement.biome == 'self' ? entity.variant : ai.movement.biome));
            }
            if (ai.movement?.flags) {
                chunks = chunks
                    .filter(chunk => !ai.movement?.flags.map((flag) => flag.startsWith('!') ?
                    !chunk.flags.includes(flag.replace('!', '')) :
                    chunk.flags.includes(flag)).includes(false));
            }
        }
        if (chunks.length)
            entity.targetPosition = Random.pick(...chunks).position;
    }
    static attackTarget(entity, target, timeout = true) {
    ...
    static selectAttackTarget(entity) {
        const range = entity.reference.entity?.viewRange || 10; // Define the attack range here (e.g., 5 units)
        const possibleTargets = this.entities.filter(target => target.id !== entity.id &&
            ...
            (entity.reference.entity.ai.attack?.targets.find(i => i.id == target.type).variant ?
                ...
                : true) &&
            new Vector3(entity.position.x, entity.position.y, entity.position.z).distanceTo(new Vector3(target.position.x, target.position.y, target.position.z)) <= range).sort((a, b) => new Vector3(entity.position.x, entity.position.y, entity.position.z).distanceTo(new Vector3(b.position.x, b.position.y, b.position.z)) - new Vector3(entity.position.x, entity.position.y, entity.position.z).distanceTo(new Vector3(a.position.x, a.position.y, a.position.z)));
    ...
    static thinkNoAttackTarget(entity) {
        ...
        if (entity.restTime.current > entity.restTime.currentMax) {
            ...
            if (!entity.init) {
                entity.init = true;
                if (Random.from(0, 3) === 2)
                    Entities.selectRandomTarget(entity);
            }
            Entities.think(entity);
        }
    }
    static thinkAttackTarget(entity) {
        if (entity.attackTarget) {
            ...
            const newPosition = new Vector3(entity.attackTarget.position.x, entity.attackTarget.position.y, entity.attackTarget.position.z);
            ...
            if (distance < reach) {
                ...
                if (entity.reference.entity.ai?.attack?.projectile) {
                    ...
                    if (entity.data.projectileTimeout == undefined)
                        entity.data.projectileTimeout = 0;
                    if (entity.data.projectileTimeout <= 0) {
                        entity.data.projectileTimeout = entity.reference.entity.ai?.attack?.projectile.timeout || 100;
                        Projectiles.createProjectile(entity, direction, entity.reference.entity.ai?.attack?.projectile.speed, entity.damage, entity.reference.entity.ai?.attack?.projectile.object, 100).on('hit', ({ target }) => {
                            if (target)
                                this.attackTarget(entity, target, false);
                        });
                    }
                    else {
            ...
            if (!Entities.selectAttackTarget(entity)) {
                if (entity.targetPosition)
                    ...
                    Entities.thinkNoAttackTarget(entity);
            }
        }
    }
    static think(entity) {
    ...
    static directDamageEntity(entity, damage, timeout = 300) {
        if (entity.data.directDamageTimeout == null)
    ...
    static update() {
        const entitiesWithAi = this.entities.filter(e => e.reference.entity?.ai && e.data.ai !== false);
        ...
        this.entities.forEach(entity => {
            ...
            if (entity.stepOn !== pos && closest)
                closest.emit('stepStart', { target: entity });
            entity.stepOn = pos;
            if (closest) {
