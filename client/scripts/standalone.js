import { Project, PhysicsLoader, THREE, ExtendedObject3D, PointerLock, PointerDrag } from 'enable3d';
...
import { ParticleSystem } from './modules/particle';
...
class MainScene extends CustomScene {
    ...
    setupControls() {
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (keyEvents['Down' + e.key])
                keyEvents['Down' + e.key]();
        });
    ...
    changeCameraAngle() {
        if (this.cameraPosition.lookat)
            return;
        this.cameraPosition.current++;
        if (this.cameraPosition.current >= this.cameraPosition.angles.length)
            this.cameraPosition.current = 0;
        this.cameraPosition.offset = this.cameraPosition.angles[this.cameraPosition.current];
    }
    updateCameraLocation() {
        ...
        if (this.cameraPosition.lookat)
            this.camera.lookAt(playerPosition.clone().add(this.cameraPosition.lookat).applyEuler(this.player.mesh.rotation));
        else
    ...
    update() {
        this.entities.think();
        if (this.firstPersonMode)
            firstPersonControls(this, this.player);
        else
