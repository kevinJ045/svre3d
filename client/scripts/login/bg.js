...
export const Running2dGoobers = (canvas) => {
    ...
    class Square {
        ...
        update() {
            ...
            if (this.gridX < 0)
                this.gridX = canvas.width / gridSize - 1;
            if (this.gridX >= canvas.width / gridSize)
                this.gridX = 0;
            if (this.gridY < 0)
                this.gridY = canvas.height / gridSize - 1;
            if (this.gridY >= canvas.height / gridSize)
                this.gridY = 0;
            if (Math.random() < 0.1) {
                this.direction = this.getRandomDirection();
            }
...
export const LoginBackground = () => {
    ...
    return (React.createElement("div", { className: 'login-bg' },
        React.createElement("canvas", { width: window.innerWidth, height: window.innerHeight, ref: canvasRef })));
};
