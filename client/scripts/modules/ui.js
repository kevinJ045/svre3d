...
export class UI {
    constructor() {
        this.menuElement = document.getElementById('full-menu');
        this.activeTab = "";
        this.inventory = {
            loadedFirst: false
        };
        this.loadedForPosition = "";
    ...
    updateItemInfo({ x = 0, y = 0, parent, item }) {
        const info = this.find('.inventory-item-info');
        if (x == y && x == 0) {
            ...
            if (item) {
                ...
                info.querySelector('.item-about').innerText = item.data.content || "";
                const actionsEl = info.querySelector('.item-actions');
    ...
    setPlayer(player) {
        ...
        const inventoryItems = [...this.findAll('.inventory-slot:not(.independent)')];
            ...
            slot.addEventListener('mouseenter', (e) => {
                if (slot.querySelector('.item')) {
                    this.updateItemInfo({
                        ...
                        parent: slot,
                        item: slot.querySelector('.item').item
                    });
                }
            });
        ...
        const addItem = (item, index) => {
            if (!item.data.inventory)
                item.data.inventory = this.createItemInventory(item);
            inventoryItems[index].appendChild(item.data.inventory);
        ...
        player.onInventory('add', (item) => {
            let e = emptyOne();
            addItem(item, e == -1 ? item.data.inventory?.index || 0 : e);
        });
        player.onInventory('update-count', (item) => {
            updateCount(item);
        });
        player.onInventory('remove', (item) => {
            removeItem(item);
        });
        player.onInventory('wear', (item) => {
            wearItem(item);
        });
    ...
    initCrafting() {
        ...
        const contentRename = this.find('#item-content-text');
        const slot_items = {
            0: [],
            1: []
        };
        ...
        const slots_update = () => {
            ...
            if (slot_items[0].length && !slot_items[1].length) {
                ...
                resultSlot.querySelector('.inventory-slot').innerHTML = '';
                const item1 = this.player.inventory.find(i => i.id == slot_items[0][1].id);
                const item2 = this.player.inventory.find(i => i.id == slot_items[1][1].id);
                if (item2.item.config.powergem == true && item1.item.accessory && item1.item.config.powergems != false && (item1.item.config.exclude_gems ? !item1.item.config.exclude_gems.includes(item2.item.id) : true)) {
                    ...
                    i.addEventListener('click', () => {
                        ...
                        if (contentRename.value && contentRename.value != item1.data.content)
                            item1.data.content = contentRename.value;
                        finishCraft();
                    });
                    resultSlot.querySelector('.inventory-slot').appendChild(i);
                }
        ...
        this.player.onInventory('update-count', (item) => {
            slots.forEach(slot => {
                const i = slot.querySelector('.item');
                if (i) {
                    if (item.id == i.item.id) {
                        i.querySelector('.item-count').innerText = item.count;
                    }
                }
            });
        });
        this.player.onInventory('remove', (item) => {
            slots.forEach((slot, index) => {
                const i = slot.querySelector('.item');
                if (i) {
                    if (item.id == i.item.id) {
    ...
    showChooseItemUi(parent, cb = (item, itemElt) => { }, ignore) {
