...
export const resolvePath = (item, suffix = 'res') => {
    return '/resources/' + item.manifest.id.split(':').join('/') + '/' + suffix;
};
export const loadItem = async (item, scene, suffix = 'res', parent) => {
    let load = null;
    const type = item.resource.loader;
    if (item.manifest.type.endsWith("_map")) {
        load = [];
        for (let index in item.resource.sources) {
            load.push(await loaders[type](resolvePath(parent || item, suffix ? suffix + '.' + index : undefined)));
        }
    }
...
export class ResourceMap {
    static async loadAll(scene) {
            ...
            if (item.resource) {
                if (item.resource.preload) {
                    for (let i of item.resource.preload) {
                        ...
                        let [fetchName, name] = i.match('@') ? i.split('@') : [i, i];
                        ...
                        if (arrayMatch) {
                            ...
                            let prop = propName.match('.') ? getPropStr(item, propName) : item[propName];
                            ...
                            if (independent) {
                                getPropStr(item, name.split('.').slice(0, -1).join('.'))[name.split('.').pop()] = prop.resource.load;
                            }
                        }
                    }
                }
        ...
        while (ResourceMap.queue.length)
            ResourceMap.queue.pop();
    }
    static find(id, type) {
        return ResourceMap.resources.find(i => i.manifest?.id == id && (type ? i.type == type : true));
    }
    static findLoad(id, type) {
