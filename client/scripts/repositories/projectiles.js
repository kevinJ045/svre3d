...
export default class Projectiles {
    static createProjectile(projectileData) {
        ...
        projectile.object3d = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 1), new THREE.MeshLambertMaterial({ color: '#ff00ff' }));
        projectile.object3d.position.set(projectile.position.x, projectile.position.y, projectile.position.z);
        SceneManager.scene.add.existing(projectile.object3d);
        this.projectiles.push(projectile);
    }
    static moveProjectile(id, position) {
    ...
    static ping() {
        pingFrom('projectile:create', (projectile) => {
            ...
            if (owner) {
                ...
                owner.object3d.position.set(projectile.position.x, projectile.position.y, projectile.position.z);
            }
        });
