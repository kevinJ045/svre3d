...
const resolveMaterial = (m, scene) => typeof m == 'string' ? materialParser(m, scene, {}) : m;
export class Goober extends Entity {
    ...
    selectRandomTarget() {
        ...
        if (chunks.length)
            this.targetLocation = Utils.pickRandom(...chunks).position.clone();
    }
    thinkNoAttackTarget() {
        this.findEnemies();
