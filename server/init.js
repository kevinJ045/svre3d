...
export async function userConnected(serverData, socket) {
    ...
    if (username) {
        ...
        if (!Entities.entities.find(i => i.data.username == username) || socket.handshake.query.reconnect != 'true')
        ...
        socket.on('register', async ({ variant, username, password, email }, cb) => {
            if (!variants.includes(variant))
                return cb(null);
            await LoginManager.register(username, password, variant, email, Chunks.findSafeSpawnPoint(variant) || { x: 0, z: 0 });
            cb(true);
        });
